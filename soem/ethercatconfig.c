...
static void ecx_set_slaves_to_default(ecx_contextt *context)
{
   ...
   ecx_BWR(context->port, 0x0000, ECT_REG_FMMU0       , 32 * 8    , &zbuf, EC_TIMEOUTRET3);  /* reset FMMU's */
   ecx_BWR(context->port, 0x0000, ECT_REG_SM0         , 32 * 8     , &zbuf, EC_TIMEOUTRET3);  /* reset SyncM */

                // 0x910
   ecx_BWR(context->port, 0x0000, ECT_REG_DCSYSTIME   , 32, &zbuf, EC_TIMEOUTRET3);  /* reset system time+ofs */


   b = 0x00;    // 0x981
   ecx_BWR(context->port, 0x0000, ECT_REG_DCSYNCACT   , sizeof(b) , &b, EC_TIMEOUTRET3);     /* reset activation register */

   w = htoes(0x1000); // 0x930
   ecx_BWR(context->port, 0x0000, ECT_REG_DCSPEEDCNT  , sizeof(w) , &w, EC_TIMEOUTRET3);     /* DC speedstart */

   w = htoes(0x0c00); // 0x934
